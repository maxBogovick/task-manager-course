...
class TaskExecutionStage02Tests {
    ...
    void shouldReturn404WhenTaskDoesNotExist() throws Exception {
        String responseBody = mockMvc.perform(post("/api/tasks/99999/executions")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new TaskExecutionRequest("No parent task"))))
        ...
        assertThat(error.message()).contains("99999");
    }

    @Test
    void shouldReturn400ForBlankOutput() throws Exception {
        String createdTaskBody = mockMvc.perform(post("/api/tasks")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new TaskDefinitionRequest("Task for validation", null))))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString();
        ...
        String responseBody = mockMvc.perform(post("/api/tasks/" + task.id() + "/executions")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new TaskExecutionRequest(""))))
